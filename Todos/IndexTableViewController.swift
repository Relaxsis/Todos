...
class IndexTableViewController: UITableViewController {
    ...
    func update() {
        Alamofire.request(.GET, "https://msrylkin.herokuapp.com/", parameters: ["format":"json"]).responseCollection { (_, _, projects: [Project]?, error) in
            self.projects = projects!
           //println(self.projects.count)
            self.tableView.reloadData()
            self.addTodo.enabled = true
        }
    }
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier == "TransferSegue1" {
            let navVC = segue.destinationViewController as! UINavigationController
            let tableVC = navVC.viewControllers.first as! NewTodoTableViewController
            tableVC.projects = self.projects}
    }
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        cell.configureCell(self.projects[indexPath.section].todos[indexPath.row])
        return cell
    }
    ...
    /*
    override func tableView(tableView: UITableView,  indexPath: NSIndexPath) -> UITableViewCell {
